...
class Return_Behav (PeriodicBehaviour): 
    def generate_exponential_decay_weights(self, n, decay_factor=0.25):
    ...
    def determine_return_quantities(self, products_to_return, quantity_decay_factor=0.25):
    ...
    async def run(self):
        if self.agent.productsBought:
            ...
            print("Client {} return {} product(s) to StockManager {}".format(str(self.agent.jid), str(tamanho), str(self.agent.get("stockmanager_contact"))))
            await self.send(msg)
